$(document).ready(function(){
    ...
    $(".tangSoLuong").click(function(){
        var idSP = $(this).attr("idSP");
        $.get("http://localhost/admin/Cart/UpdateQuantity/"+idSP+"/1", function(data){
            var ajax = JSON.parse(data);
            //console.log(ajax.SOLUONG);
            $(".soluong[idSP='"+idSP+"']").html(ajax.SOLUONG);
            $(".dongia[idSP='"+idSP+"']").html(ajax.THANHTIEN);
            $("#tongtien").html(ajax.TONGTIEN);
        });
    });

    $(".giamSoLuong").click(function(){
        var idSP = $(this).attr("idSP");
        $.get("http://localhost/admin/Cart/UpdateQuantity/"+idSP+"/0", function(data){
            var ajax = JSON.parse(data);
            //console.log(ajax.SOLUONG);
            $(".soluong[idSP='"+idSP+"']").html(ajax.SOLUONG);
            $(".dongia[idSP='"+idSP+"']").html(ajax.THANHTIEN);
